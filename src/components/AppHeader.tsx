...
export const AppHeader: React.FC<AppHeaderProps> = ({ 
  ...
  useEffect(() => {
    const updateCenterPosition = () => {
      ...
      // В этом случае всегда используем процентные fallback значения для адаптивности
      const isRoot = location.pathname === '/';
      const windowWidth = window.innerWidth;
      
      // Используем процентные значения для fallback (адаптивность под разные ширины экрана)
      // На основе визуального анализа: SettingsButton занимает ~8-10% ширины экрана
      // BackButton занимает ~10-12% ширины экрана с текстом "Назад"
      ...
      if (safeLeft === 0 && safeRight === 0) {
        ...
        // Используем процентное значение вместо фиксированных пикселей
        safeRight = (windowWidth * 0.09); // ~9% от ширины экрана (примерно 54px на экране 600px)
      ...
      // Используем проценты для полной адаптивности под разные ширины экранов
      // Унифицированная формула для всех страниц
      
      // Конвертируем пиксели в проценты для вычислений
