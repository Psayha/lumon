...
**Chat Workflows:**
- ⏳ `chat.create` - создание чата (базовый файл есть, требует обновления)
- ⏳ `chat.get-history` - получение истории (требует auth.validate)
- ⏳ `chat.save-message` - сохранение сообщения (требует auth.validate)
...
### Этап 3: n8n - Обновление существующих workflows ⏳ В РАБОТЕ
