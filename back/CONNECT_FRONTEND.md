...
### 0. ⚠️ ВАЖНО: Обновить nginx конфигурацию

**Проблема:** Без правильной конфигурации nginx будет возвращать 405 Not Allowed для `/webhook/*` запросов.
...
Или вручную:
```bash
sudo cp nginx-frontend.conf /etc/nginx/sites-available/lumon-frontend
sudo nginx -t
sudo systemctl reload nginx
```

**Что добавлено:** блок проксирования для `/webhook/` → `https://n8n.psayha.ru`

Подробности в [`FIX_405_ERROR.md`](./FIX_405_ERROR.md)
...
- [ ] ⚠️ **КРИТИЧНО:** Обновлена nginx конфигурация на сервере (иначе 405 ошибка)
